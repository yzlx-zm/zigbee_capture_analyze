...
- [U7 拓扑页优化](issues/U7-拓扑页优化.md) — 形状分类 (协调器六边/路由菱形/终端圆/未知三角) + 死控件修复 (taddr 定位 / 静默节点切换) + 播放按钮 + 时间刻度条 + 图实例复用 (时间过滤不再重建, 性能提升); 截图见 .scratch/verification/u7-topo/; **终端设备判定修正 (08-06)**: 素材实证两方向误判 — ①SED 误判 router (Route Request/Record 发送者非 FFD 信号: 群控锁 0x82A0/0xD6D3, G32 0xEE48 实证 SED 也发) ②无信号整网误判终端 (test2 2628 节点); 重构为协议级信号: LS/RREP→router, MAC poll→end_device (修复 poll 帧被 NWK 过滤排除), DA capability 权威声明 (cubx), 无信号→unknown; 验证 34/34 + P1 回归 15/15; ⚠️ 需重导素材生效 (device_type 为导入时快照)
