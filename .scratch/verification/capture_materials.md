...
| `验证可用-记录\2-群控压测问题包.cubx` | 5MB | 群控压测 (16 智能锁 + 唯一中继 19950, 54,403包/43.5min/信道26, PAN 42806) | ✅ P1 解析器修复回归 (13/13): 0x28 伪命令清零 (同信道其他 PAN 帧 ×1266)、nwk_dst 广播保留、帧去重 (37% 重复捕获); **L2-1 健康基线** (16 锁 ~1s 轮询无异常); **投递率实证**: 去重后 399 事务, 54995=62%/33440=67% 缺投但 17/17 锁侧 APS ACK 佐证收到 (抓包漏投递帧, 非中继不转发); 锁无 ZCL 上行 (只回 APS ACK); 0x61/0x20 命令语义待确认 |
