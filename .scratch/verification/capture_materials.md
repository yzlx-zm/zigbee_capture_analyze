...
| `验证可用-记录\2-群控压测问题包.cubx` | 5MB | 群控压测 (16 智能锁 + 唯一中继 19950, 54,403包/43.5min/信道26, PAN 42806) | ✅ P1 解析器修复回归 (15/15): 0x28 伪命令清零 (同信道其他 PAN 帧 ×1266)、nwk_dst 广播保留、帧去重 (Unlock 27% 物理重复 + 10% 重传, 全局 8.5%); **L2-1 健康基线** (16 锁 ~1s 轮询无异常); **投递实证 (自审修正 08-06)**: 物理去重后 429 发送事件;中继锁 54995/33440 各 25 发送, 投递帧捕获 16/25 (64% = 抓包器第二跳捕获率), **缺投 18 帧 18/18 有锁侧 APS ACK (17/18 counter 匹配) → 锁实际全部收到 (中继真投递, 抓包漏帧)**;直发锁 ACK 覆盖 52-87%;锁无 ZCL 上行 (只回 APS ACK); 0x61/0x20 命令语义待确认 |
...
| `G32_0626.cubx` | 720KB | G32 网络中继错误 (运营期 36min, 中继 0xBE5A) | ✅ **L3-5-R2 验证通过** (0x0C ×216/39轮/29min); ✅ **L6-S3 验证通过** (2026-08-05: 0x06 ×38 下行投递失败型 — poll ×1933 活跃仍过期, 0x06 距 poll 平均 2.5s, 与 0x0C 交叉); MTORR 频率 7.2s/次 (异常高频) |
| `G32专项分析/08031620.cubx` | 5MB | G32 大包 (54403 包, 8月3日) | 📝 **模式 4 观察** (2026-08-05): NS=0 (无错误码) 但 **MTORR ×771/43min = 3.4s/次** (健康默认 60s) — 配置 vs 隐含路由问题待用户确认; L3-5 HEALTHY |
...
| 13 | L6-S3 睡眠型 | SED 睡眠过久未 poll 导致 0x06 (poll 缺失/间隔 ≥7.68s) — 覆盖父节点↔SED 链路 | 验证经典睡眠型 (现只有投递失败型实证) |
