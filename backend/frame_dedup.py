"""帧去重公共能力 — P1 产出 (2026-08-05, 自审修正 2026-08-06).

背景: cubx 抓包器 (Ubiqua 多接收器/冗余采集) 会把同一空中帧重复捕获多次。
素材实证 (2-群控压测问题包.cubx): Unlock 命令帧 634→461 (1s 窗同跳去重,
物理重复 27%) + 上层重传 10% (同事务不同 mac_seq, 461→399 事务); 全局帧
54,403→49,796 (8.5%)。不先去重, 投递率/失败率等计数类分析必然失真
(如群控投递率首轮计数因重复帧翻倍)。
⚠️ 事务级数字 (如"发送次数") 不能用 (nwk_src,nwk_dst,nwk_seq) 键 —
    nwk_seq 8 位回绕会错误合并独立事务 (素材实锤: 同 (54995,seq=12)
    实为相隔 421s 的两个事务, 曾致投递率 62%/67% 与 100% 全部数字失真)。
