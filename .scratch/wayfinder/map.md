...
- [U20 导入页增强三项](issues/U20-导入页增强三项.md) — **三项独立实现+独立提交 (2026-09-20)**:
  ...
  **J 一次拆多段** (f1cb3c7): `split_cubx_multi` **单次扫描**拆 N 段 (半开无重叠/末段截断/
  上限 20) + `/cubx/split-batch` + 面板批量拆控件 (段数实时预览+超额禁用); 实测群控
  43.5min→9 段 **0.5s**, **帧数和 = 源 108474 (无重叠无遗漏)**; 顺带修复 **range step=1
  量化窗口** (全窗丢末段 0.79s/27 帧, 先于本次存在) + 自身块级作用域 bug (updSegHint →
  面板 dataset 丢失); 回归 P1 15/15 · zcl_fcf 12/12 · p2 4/4 · parser_verify 12/12;
  CDP 33/33; 遗留: 真实 Ubiqua 复验 / S1 复验脚本失效 (已用 cdp_j_s1_compat 覆盖)
