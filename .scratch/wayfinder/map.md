...
- [L3-11 应用层重传频繁检测闭环](issues/16-L3-11应用层重传.md) — 文档 v1.0 + 检测器 R1 + 前端卡 (13 卡); 新 counter 轮次≥3 = 应用层重试 (同 counter=栈重传 L3-1 区分); 自审修正: 周期轮询误报 (0x4FBC/0xF342 Read 有响应) → 长间隔+成功响应排除; test2 0x89F9 ×16 轮/25.2s 实证
