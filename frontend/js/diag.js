...
reg('diag',function(){
  ...
  A.get('/api/diag/l1').then(function(l1d){
    ...
    h+='<div class="card" style="margin-bottom:12px;background:#f8fafc;border-left:3px solid #3b82f6">'
      ...
      // L1-3
      +'<div style="flex:1;min-width:280px;background:#fff;border-radius:6px;padding:10px;border:1px solid #e2e8f0">'
      +'<div style="font-weight:600;font-size:12px;margin-bottom:4px">L1-3 密钥分发: <span style="color:'+vc3+';font-weight:700">'+l3.verdict+'</span> <span style="color:#94a3b8;font-size:10px" title="置信度: 高=直接证据/中=帧模式/低=推断/不可判定=数据不足">置信度:'+l3.confidence+'</span></div>'
      +'<div style="font-size:11px;color:#475569;line-height:1.6">'
      +'入网设备: <b>'+l3.joined_device_count+'</b> 台<br>'
      +'<span style="color:#64748b">'+l3.summary+'</span>'
      +((l3.devices||[]).length ? '<div style="margin-top:4px;border-top:1px dashed #e2e8f0;padding-top:4px">'
        +(l3.devices||[]).map(function(d){
            var dc = d.verdict==='HEALTHY' ? '#16a34a' : (d.verdict==='L1-3_HIT' ? '#dc2626' : '#f59e0b');
