...
### ⑥ U12 学习机制实现窗口 (S2 诊断稳定化已完成, 场景 tab=学习容器, 2026-09-07)

```text
项目: D:\ai_agent\zigbee_capture_analyze

你是 U12 实现窗口。任务: 诊断页学习机制 (场景 tab = 学习容器, 案例库+差距报告+导出传播)。

## 开工前必读 (按序)
1. .scratch/wayfinder/README.md + .scratch/wayfinder/map.md (流程 + 现状)
2. **.scratch/wayfinder/issues/U12-诊断页学习机制.md** (核心: grilling 对齐决策 —
   场景 tab=学习容器/自动场景判断/结论由案例支撑/导出传播, 完整实现要点 + 验证标准, 按它实施)
3. S2-诊断页稳定化.md Resolution (诊断页现状 — 检测器注册表/白话化/事件链已稳)

## 工作流 (按 ticket 实现要点, 分四步推进每步验证)
1. **案例库存储** backend/case_library.py: cases/ 目录 + add/list/get/find_similar/
   export/import + 素材副本管理 (cases/materials/)
2. **相似案例检索** find_similar (场景集合 Jaccard + 根因/现象关键词 + PAN/地址; 先关键词,
   不引入向量化)
3. **场景 tab 改造** diag.js: 55 场景 tab (检测器覆盖的显示结果, 未覆盖显示"待学习");
   tab 内案例列表 + 案例数徽章 + 置信度标注 (案例≥3=多案例支撑; 少=待更多案例);
   学习进度 "已学 X/55"; 新包诊断显示相似历史案例区
4. **案例导入标注流程**: 导入问题包标注表单 (现象必填/根因建议列表/严重度/环境;
   PAN/地址/时间段自动预填) → 跑检测 → 案例入库 (自动命中+人工确认归属) → 差距报告
5. **差距报告+待完善清单** GET /api/cases/gaps + 前端 (未对齐项→检测器改进优先级)
6. **导出传播** POST /api/cases/export + import (zip: cases.json + materials/ 素材副本;
   复用 _redact 思路不含密钥; 规则不随包导出 — 代码随工具版本分发)
7. 回归: 无案例时诊断页行为与现状一致; U6 后台任务基建复用; 版本号递增
8. 收尾 (铁律 7): ticket Resolution + map.md 条目 + git commit + push (代理 127.0.0.1:7897)

## 铁律
- 一次会话只解 U12; 第一阶段不做自动修改检测器 (案例库+差距报告+人工确认闭环优先 —
  ticket 明确边界); 诚实标注 (55 tab 渲染性能/案例归属歧义等边界如实记录)
- 技术断言有依据; 遇阻塞停下汇报; 大工单分步交付每步验证汇报
- 后端: python -m backend --port 8720 (改代码需重启); CDP 验证 (Edge 9222)
- 素材: 中继入网抓包(1).cubx (标注"入网后2s被踢/源路由失效"验证入库) + TC→737D 被踢重入
  (链路变化/差距报告) + 1-标准入网抓包-2 (健康对照)
- T2 打包已交付 (8c3aab5): U12 新增的数据目录 (cases/) 注意与 %APPDATA% 数据分层一致 —
  打包后案例库属用户数据, 应在 config.APP_DATA_DIR 下
```

### ⑦ 外部反馈优化窗口 (用户收集外部使用反馈后对齐, 2026-09-18)

```text
项目: D:\ai_agent\zigbee_capture_analyze

你是 U18 实现窗口。任务: 导入页拆分面板时间窗输入体验优化 (外部使用者反馈)。

## 开工前必读 (按序)
1. .scratch/wayfinder/README.md + map.md (流程 + 现状)
2. **.scratch/wayfinder/issues/U18-导入页时间窗输入体验.md** (核心: 外部反馈原文 + 现状分析
   (含代码行号) + 4 项对齐决策 + 实现要点 + 验证标准)
3. S1-导入页稳定化.md Resolution (前期修复 — 特别是 ±60s 容差/clamp 必须保留)

## 工作流 (4 项决策逐一落地, 每项即时验证)
1. **自动生效**: 8 格全满自动应用 + 输满 2 位跳下一格 + Enter 应用; 删除"应用"按钮
   (保留 S1 的 ±60s 容差 + clamp 逻辑, 不得丢失)
2. **分段推进**: 「◀ 上一段 / 下一段 ▶」按当前窗口长度整段推进; 越界 clamp + 提示
3. **持久化**: localStorage 存 {ts_first, winStart, winEnd, inputs, subs}; ts_first 不匹配丢弃
   (防串包); 切页 + 刷新都恢复
4. **子包清单一并持久化** (双写: 内存供渲染 + localStorage 供刷新恢复)
5. import.js 改动 → **index.html/app.js 版本号递增** (项目铁律, 否则用户刷不到)
6. CDP 验证 (Edge 9222): 自动生效/推进按钮/切页恢复/刷新恢复/换素材不串包/非法输入
7. 收尾 (铁律 7): ticket Resolution + map.md 条目 + git commit + push (代理 127.0.0.1:7897)

## 铁律
- 一次会话只解 U18; 诚实标注 (刷新恢复实测结果 / 边界行为)
- 技术断言有依据; 遇阻塞停下汇报
- **回归**: 拆分流程 (拆分子包/下载/导入子包) 与 S1 全部修复项不受影响
- 后端: python -m backend --port 8720 (改代码需重启); 素材: 任一大包 (>1MB 触发面板,
...
### ⑩ U21 拓扑放射布局窗口 (节点多看不清 → 放射+终端聚合, 2026-09-21)

```text
项目: D:\ai_agent\zigbee_capture_analyze

你是 U21 实现窗口。任务: 拓扑图改放射状布局 (协调器圆心, 半径=跳数分环) + 终端聚合,
解决"节点一多看不清"。

## 开工前必读 (按序)
1. .scratch/wayfinder/README.md + map.md
2. **.scratch/wayfinder/issues/U21-拓扑放射布局与终端聚合.md** (核心: 参考图形态/现状分析
   (含代码行号)/4 项对齐决策/实现要点/验证标准/风险)
3. 参考图: D:\work\rexense-瑞瀛\压测记录\32中继设备控制压测\网关拓扑.png (网关厂商的
   放射星形拓扑 — 目标观感)
4. frontend/js/topo.js 布局段 (行 ~752-773 现有列式/网格; nodeDepth BFS 已存在) +
   backend/api/topology.py `_link_evidence_parent` (父链路证据)

## 工作流 (建议分步, 每步出图对比)
1. **放射布局**: 圆心=协调器, 半径=跳数; **角度用子树扇区分配** (朴素均分会导致边交叉)
   — 先用真实素材 (中继入网(1) 小网络 → 群控 85 节点) 出图确认再继续
2. **终端聚合**: 路由/协调器骨架 + 终端 ×N 徽章 (徽章汇总内部状态 ⚠️/⛔);
   小网络 (<~50) 自动全展开; 搜索/聚焦终端 → 自动展开所在簇
   ⚠️ 避免 Cytoscape compound (parent 是保留字, U13 踩过坑) — 用普通节点+特殊样式
3. **布局切换**: 顶栏加 放射(默认)/列式(现有)/自由 三档; 现有布局代码保留
4. **标签**: 外侧 + 密度自适应 (密集只显路由标签, 其余 tooltip)
5. **关键约束**: 时刻游标/时间过滤**只改样式不重算位置** (否则拖动时节点乱跳)
6. 验证: 小/中/大三档网络 (6-11 / 85 / 401-2628 节点) + 星形网络形态接近参考图
   + 交互回归 (时刻游标不跳/搜索展开/布局切换/图例·静默·邻居边)
7. CDP 截图核对 (对照参考图观感); 版本号递增
8. 收尾 (铁律 7): ticket Resolution + map.md 条目 + commit + push

## 铁律
- 一次会话只解 U21; 现有拓扑能力 (形状/行为状态/链路证据边/时刻游标/图例/路径面板)
  **全部保留不得回归**; 诚实标注 (大网络渲染耗时实测数字)
- 技术断言有依据; 遇阻塞/大网络卡死 → 停下汇报 (跳数限制作为备选, 本次不做)
- 后端: python -m backend --port 8720; 素材: 中继入网抓包(1) / 群控压测包 /
  test2 (401-2628 节点大网络)
```
...
| U14 | 拓扑节点显示增强 | UI 工程 (发散对齐产出, 2026-08-24) |
| U15 | 节点控制协议解析与导出 | UI 工程 (grilling 对齐产出, 2026-08-24) |
...
| U21 | 拓扑放射布局 + 终端聚合 | UI 反馈优化 (用户反馈+参考图, 2026-09-21) |
