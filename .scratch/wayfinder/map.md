...
- [U8 诊断页优化](issues/U8-诊断页优化.md) — 批次 1 完成 (08-10): **diag.js 嵌套 → 检测器注册表** (MODULES 数据驱动, 加检测 = 注册条目+render 函数); **自审修正**: Promise.all 统一渲染 → 渐进渲染+15s 超时兜底 (单模块挂起整页空白实测复现); CDP 对比逐字符一致; **批次 2 完成 (08-10)**: 设备地址 ⏱/🗺 跳转 (S.topoAddr 契约, U4 联动落地) + **跨卡片事件链卡** (hitDevices 登记, ≥2 命中触发; CE77: L1-3×L1-4×OFF 案例验证); 待做: 批次 3 覆盖提示
