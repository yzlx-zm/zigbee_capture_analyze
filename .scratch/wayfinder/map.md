...
- [U20 导入页增强三项](issues/U20-导入页增强三项.md) — **三项独立实现+独立提交 (2026-09-20)**:
  **H 密钥同步全类型** (d4f1f1a): `list_typed_keys` (Network+Link) + 类型标签/计数 +
  `POST /keys/refresh-ubiqua` (未运行 → 503 提示) + 面板刷新按钮/类型徽章 + 结果卡同步行;
  实测解密率 test2 **0.0%→11.1%** (9303 帧) / 标准入网 9.9%→11.6%; LinkKey 覆盖度实验
  **无正收益** (设备 link key 不在素材 Keys 表内, 如实标注); 顺带修复 `zigbee_pc_keys`
  预设 key 重复读出 (用户文件 356 行/321 hex, 预设重复 36 次; 读侧修复+自愈, 未擅改文件);
  ⚠️ U19 未实现 + Ubiqua 本机未运行 → 用 55 素材 cubx Keys 表 schema 实证 + mock 服务
  (真实密钥源) 验证, 真实 Ubiqua 复验待用户环境
  **I 按 PAN 过滤** (a2a7584): 轻量 MAC 头 PAN 提取 (源码级对齐 scapy) + `zigbee_relevant`
  (排除必不进数据的异协议帧) + 预扫 PAN 分布 (0.11s/10.8 万帧) + 拆分/导入 pan 过滤 +
  面板 PAN 下拉 (降序/建议/默认全量/单 PAN 不显示); 对账: 主 PAN 计数一致, 残余
  ±1~2 帧 (≤0.25%, scapy 同样拒绝寻址的畸形帧); 实测中继包选 0x580C → 2176 帧
  **解密率 41.7%→100%** (数据纯净度价值)
  **J 一次拆多段 — ⚠️ 已撤回** (f1cb3c7 实现 → **7cfbe3c 撤回**): 用户 2026-09-20 反馈
  "功能点意义不明, 我感觉要去掉" → 删面板批量拆控件 + `/cubx/split-batch` +
  `split_cubx_multi` (完整实现留在 git 历史 f1cb3c7, 需要可取回); **保留**该验证暴露的
  两个真修复: ①range step=1 量化窗口 (既有"拆分子包"流程全窗丢末段 0.79s/27 帧)
  ②runSplit 公共运行器; 复验: 既有拆分 8626/2176 正常 + CDP 8/8;
  回归 P1 15/15 · zcl_fcf 12/12 · p2 4/4 · parser_verify 12/12;
  遗留: 真实 Ubiqua 复验 / S1 复验脚本失效 (已用 cdp_j_s1_compat 覆盖)
...
- [U21 拓扑放射布局 + 终端聚合](issues/U21-拓扑放射布局与终端聚合.md) — **交付 (2026-09-21)**:
  放射布局 (圆心=协调器, 半径=跳数, **子树扇区**分配角度 + 环半径=gap/最小相邻角距 解析式) +
  终端聚合 (徽章 ×N 汇总簇内状态, 簇≥2 台才折叠, 普通节点非 compound) + 三档布局
  (放射默认/列式保留/自由) + 标签外侧密度自适应 (弦距<96 单行 / <46 非路由隐藏);
  **关键约束**: 层级只取 link_evidence_parent 父字段 (与 curT 无关) → 拖时刻游标位置零位移
  (实测 0px); 孤儿 → 最外环独立分环; 实测 4 素材: 34 节点星形 16/16 · 10 节点 15/15 ·
  21 节点 13/13 · 92 节点 13/13 (首屏 580ms/布局 11-20ms/零交叉/零标签重叠) ·
  ...
  **U21-2 反馈轮 (2026-09-21 晚, 提交 0029941)**: 用户反馈"环上距离都不相等" → 根因 =
  扇区权重按**子树规模** (中继子树 15 独占 300°, 环1 角距 [160,20,20,160]); 对照实验
  (模拟多级 44 节点 / 真实 test2 92 节点 4 级 / IR22) 后用户裁定 **hybrid**: 第 1 环 α=0.5
  平方根平滑, 深层 α=1 保紧凑 (纯 α=0.5 会让深层环胀 28%; α=0 全均分 = 灾难 环4→9358px);
  顺带修真 bug: **聚合态孤儿环半径塌陷** (baseR 取 tree.dep 最大深度 → 折叠后最深层无环半径
  → 孤儿退到 120px 压在同位的环上节点身上, 探针实锤) + 徽章与父标签擦边 2-6px + 验证脚本
  共线退化误报 (test2 1 条假阳性已用四点同角+半径区间证伪); 关系判定帧级查证 (12/14 终端
  100% poll 中继 · 3 台弱节点父在抓包盲区 → 是否"父不在图→退到可见证据段"待用户裁定);
  复验 IR22 19/19 · test2 12/13; 对比图 u21_ml_*.png / u21_angle_*.png
  **U21-3 标签外侧修复 (提交 317021a)**: 用户反馈"地址显示在节点图标内部" → Cytoscape 语义
  实测 (最小图探针): `text-halign:'left'` = 标签在节点左侧, 且 `text-margin-x/y` 恒按屏幕方向
  偏移 (不随对齐翻转) — 旧代码映射写反 → 右半/上半节点标签压在图标上; 同时补上**验证盲区**:
  新增"标签必须在节点外侧"断言 (径向最远点) + 横向/纵向分支覆盖 + 破坏性测试 (人为错映射必报错)。
- [U22 拓扑时刻态节点标记](issues/U22-拓扑时刻态节点标记.md) — **交付 (2026-09-24)**:
  节点状态由"整段 behavior 汇总"改为**游标时刻 T 的证据状态** (正常/🔄重入网中/⛻已离网/⏳无证据);
  后端 `_moment_evidence` 单趟扫描出事件时间线+无证据区间 (走 PAN 过滤); 视觉复用既有语义;
  真实素材 (中继入网: leave×340/announce×35) 验证 5/5 (0x737D 离网 ⛻ / 0x838D 缺口 ⏳);
  协调器豁免 (其 Leave 是踢人); 事件优先于缺口; 版本 20260924c;
  遗留: `_behavior_map` 仍无 PAN 过滤 (旧字段供节点页/诊断页, 建议下个 ticket)
- [U25 拓扑底部面板收起/缩放](issues/U25-拓扑底部面板收起缩放.md) — **交付 (2026-09-24)**:
  底部面板加"▾ 收起/▴ 展开" + 上沿 9px 拖拽把手 (40px~72% 视口), 状态存 localStorage;
  实测收起后面板 220→34px、绘图区 597→**783px**、fit 0.886 (节点更清楚);
  收起态点 tab 自动展开; 版本 20260924b; CDP u25_panel 7/7
- [U24 拓扑间距收紧 + 时刻排布](issues/U24-拓扑间距与时刻排布.md) — **交付 (2026-09-24)**:
  ① 间距: 外环半径 1251→**508**, fit 0.61→**0.86**, 9px 字号上屏 4.8→**8px** —
  根因是标签**切向占用按朝向**区分 (左右两侧标签沿半径外伸, 切向只占行高) + 布局按双行预留
  而标签 pass 又降级单行 (白撑); 现在两处**共用同一模式判定** (双行96/单行46/隐藏34) 只收紧迭代;
  ② 时刻排布: 父表改**时刻 T 的链路** (link_snapshots, parent 段优先) → 环位/角度/折叠随时刻
  重算 = "完全跟随" (实测每时刻图都紧凑; 代价: 链路变则整图重排, 中位 64px);
  **稳定版已试并弃用** (静态角度+只移跳数变节点 → 结构差异大的时刻角度聚类, 外环被迫 1261px
  fit 0.4, 反噬①); 顺带修 3 个真 bug (单成员簇徽章 ×1 重复表示 / 徽章偏移非确定性 /
  时刻环半径未级联); 回归 u21_net 14/14 · u21_agg 20/20 · label 5/5 · u24_moment 5/5;
  残留: 聚合态 2 处 2~4px 标签盒重叠 (对三组参数完全不敏感 = padding 级, 肉眼不可辨)
- [U23 拓扑页大包加载性能](issues/U23-拓扑页大包加载性能.md) — **已交付 (2026-09-22)**: 首屏 **20.7s→1.94s**,
  events 冷 **19.8s→67ms** (导入后后台预计算主 PAN), 热 **0.53s/5.1MB→0.10s/311KB** (段按
  **链路身份**合并: 42256→2626 段), 冷算期间其它请求 **19.8s→0.10s** (端点走线程池+计算锁),
  语义 8000 采样点逐点对账 **0 不一致**; 遗留: 冷算本身仍 ~18s (仅切非主 PAN/重启后触发),
  GIL 争用待移进程; 原记录:
  81MB/173 万帧素材导入后首次进拓扑页 **首屏 20.7s** (其中 events 请求 19.8s 冷 / 12.7s 复现),
  且期间**其它请求被拖到 19.8s** (并发 import/status 实测) = 用户"卡顿"感受的真因;
  前端不是瓶颈 (布局 2~3ms); payload 5.1MB (link_snapshots 14441 段未合并);
  根因 = events 端点 8 遍全量扫描 (`_link_evidence_parent` 还被重复调用);
  方案 (推荐 1+3+5): ①导入时预计算+持久化 (页面只请求全量无窗 → 可缓存) ③payload 段合并瘦身
  ⑤前端骨架+进度; 另 ②合并扫描 ④重活挪出事件循环; 梳理文档 docs/topology_page_review.md
- [U22 拓扑时刻态: 节点证据状态标记](issues/U22-拓扑时刻态节点标记.md) — **口径已对齐, 未开工**
  (2026-09-21 /grilling, 代码未动): 用户目的 = "时刻态拓扑 + 能看到什么时候出异常 (rejoin/leave)";
  用户裁定 **只做节点时刻态标记** (不做事件带/事件列表); 现状 5 问题查实 (rejoining 粘性无时间窗 /
  online 与 behavior 双轨可矛盾 / offline 与 S3"静默≠离线"冲突 / sleeping 不可达 / `_behavior_map`
  **未走 PAN 过滤** = 多 PAN 串网同源 bug); 冻结口径 = 四态 (正常/重入网中/已离网/无证据) 依据
  CONTEXT.md 生命周期状态机 + 事件全为设备自报帧 (硬证据), "离网"只留给 Leave(rejoin=0) 硬证据,
  无证据只陈述"证据中断"; 阈值 N=60s / 缺口 max(4×poll间隔,60s) 待实测定; 术语已写入 CONTEXT.md

## Not yet specified

- **free_port() 端口选择 bug** (2026-09-21 发现, 用户裁定"先只记录不改"):
  `backend/__main__.py:23` `def free_port(default: int = 8720)` 的 **default 参数未被使用**
  — 函数内直接 `bind(('',0))` 让系统随机分配端口。后果: 不带 `--port` 启动时端口是随机的
  (非 8720), 与文档/直觉不符; 手工命令行启动的用户不知道端口 → 误判"输入端口无效"
  (打包版会自动开浏览器所以不易察觉)。**修法**: 先试 bind 8720 (SO_REUSEADDR 语义注意),
  失败再回退 `bind(('',0))`; 属启动路径小改, 建议随下次打包一起做并复验
  (含"8720 被占用时回退随机"与"双实例锁"两条负例)
- **单实例锁排查体验** (2026-09-21, 设计如此非 bug): 锁不分端口, 第二实例只打印
  "工具已在运行, 请关闭已打开的窗口 (控制台窗) 后重试." 后退出 (实测 `--port 8899` 无监听)。
  建议改进: 该提示附"当前实例 PID/端口", 用户可自助定位已在跑的实例
- U20 遗留 (2026-09-20): ①真实 Ubiqua `/keys` 复验 (H 的 mock 之外) ②精确时间输入
  末分钟语义 (输入 16:20 截到 16:20:00, 丢该分钟剩余秒) 待用户裁定 ③S1 复验脚本
  s1_verify2.mjs 失效 (引用已删 CSV tab + 缺失 fixture) 待重写 ④U18 (时间窗体验) /
  U19 (Ubiqua 直连导入) 仍未实现 — 本次 U20 已在现状面板上做加法, 与两者无同区冲突
...
## 路线备忘 (G 系列增强项, 2026-09-07 记录, 按需固化不开窗口)

> v1.0 已交付 (S1-S4 稳定化 + T2 打包 1.0.1, U12 in-progress)。以下为工具进化方向备忘, 需要时再固化工单。

- **G1 统一证据等级** — verified/supported/inferred/unknown/not_enough_data/not_decrypted 贯穿诊断/拓扑/报文/节点/AI/导出; U12 落地后做 (案例库是分级素材源)
- **G2 统一数据契约** — 原始/解析/派生/显示字段分层 + source/scope/confidence 标记; 先字段清单+口径说明+契约测试, 不大规模重构
- **G3 大包索引与缓存** — 导入后建索引 (时间/PAN/地址/层/cluster/APS counter), 报文分页/节点统计/拓扑聚合/AI 取数免全量扫描; 第二阶段: 任务取消/断点恢复/流式
- **G4 拓扑链路质量与证据展示** — 边显示绘制原因 (证据类型/时间/强度) + LQI/RSSI/Link Cost/质量等级分层; 禁止 RSSI→ETX/通信量→父节点等无依据推断
- **U10 AI 导出集成** (待确认) — 导入页一键导出 AI 数据集, 复用 U6 后台任务基建
- **U17 收尾** — 真流式 LLM key 实测 + 回答来源分类 (📘官方/📊实证/⚠️推断) + 不可信输入安全边界
- **S5/S6/S7** — 节点页/AI 侧边栏/顶栏全局 稳定化 (S0 计划内未开)
